...
describe Audioreleaser::Tagger do
  before do
    ...
    @tags = Audioreleaser::Tags.new(
      album: 'Tłuczące pokrowce jeżozwierza',
      artist: 'Jęczące Brzękodźwięki',
      album_artist: 'Różni artyści',
      year: 2021,
      title: 'Przebój',
      rank: 2,
      comment: <<~COMMENT.chop,
  ...
  it 'sets artist tag' do
    @tagger.apply_to(@flac_path, tags: @tags)
    ...
    @tagger.apply_to(@ogg_path, tags: @tags)
    ...
    @tagger.apply_to(@mp3_path, tags: @tags)
  ...
  it 'adds comment tag' do
    @tagger.apply_to(@flac_path, tags: @tags)
    fetch_tags(@flac_path) { |found| assert_equal @tags.comment, found.comment }
    assert_equal @tags.comment, @tagger.fetch_from(@flac_path).comment

    @tagger.apply_to(@ogg_path, tags: @tags)
    fetch_tags(@ogg_path) { |found| assert_equal @tags.comment, found.comment }
    assert_equal @tags.comment, @tagger.fetch_from(@ogg_path).comment

    @tagger.apply_to(@mp3_path, tags: @tags)
    fetch_tags(@mp3_path) { |found| assert_equal @tags.comment, found.comment }
    assert_equal @tags.comment, @tagger.fetch_from(@mp3_path).comment
  end
